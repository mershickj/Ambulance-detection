...
# -------- LANE POLYGONS (adjust if needed) --------
LEFT_LANE = np.array([
    (0, int(height * 0.55)),
...
while True:
    ...
    for box, cls in zip(results.boxes.xyxy, results.boxes.cls):
        ...
        # -------- LABEL NORMALIZATION --------
        if raw_label in ["motorbike", "motorcycle"]:
            label = "bike"
        ...
        elif raw_label == "ambulance":
            label = "ambulance"

        elif raw_label == "truck":
            # 🔴 AUTO vs TRUCK FIX
            # Autos are small → classify as AUTO
    ...
    cv2.putText(frame,
        ...
        (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 255), 2)
    ...
    cv2.imshow("Lane & Class Wise Vehicle Counting", frame)
